...
const proposalSchema = zod_1.z.object({
    title: zod_1.z.string().min(10, 'Title must be at least 10 characters').max(500, 'Title too long'),
    description: zod_1.z.string().min(50, 'Description must be at least 50 characters').max(5000, 'Description too long'),
});
const updateProposalSchema = zod_1.z.object({
    title: zod_1.z.string().min(10).max(500).optional(),
    description: zod_1.z.string().min(50).optional(),
    status: zod_1.z.enum(['OPEN', 'CLOSED', 'ARCHIVED']).optional(),
});
const voteSchema = zod_1.z.object({
    proposalId: zod_1.z.string().uuid('Invalid proposal ID'),
});
