...
describe('Vote Service Security Tests', () => {
    ...
    describe('Rate Limiting (conceptual)', () => {
        it('should prevent rapid successive votes from same user', () => {
            ...
            for (let i = 0; i < 10; i++) {
                const now = Date.now();
                const recentVotes = voteAttempts.filter(v => now - v.timestamp < RATE_LIMIT_WINDOW);
...
//# sourceMappingURL=voteServiceSecurity.test.js.map