...
test.describe('Proposals', () => {
  test('authenticated citizen can submit a proposal through the multi-step form', async ({ page }) => {
    // Log in as citizen
    ...
    // Verify success message or new proposal in list
    await expect(page.locator('text=New Community Garden')).toBeVisible();
  });
  ...
  test('proposals feed filters by status', async ({ page }) => {
    // Navigate to /proposals (unauthenticated)
  ...
  test('unauthenticated user sees login prompt when trying to vote', async ({ page }) => {
    // Navigate to proposal without logging in
    await page.goto('/proposals/prp-001');
    
    // Try to find vote button (should not be clickable or should redirect)
    const voteButton = page.locator('button:has-text("Vote for this Proposal")');
    
    // If button exists, clicking should redirect to login
